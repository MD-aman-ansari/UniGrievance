...
export const LoginPage: React.FC = () => {
  ...
  const isEmailValid = emailRegex.test(trimmedEmail) && !trimmedEmail.includes('..');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEmailValid) {
      setError('Email failed verification: Please enter a valid institutional email address (e.g. name@campus.edu).');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {email.length > 0 && (
                <span className={`text-[10px] font-semibold flex items-center gap-0.5 ${isEmailValid ? 'text-emerald-600' : 'text-amber-600'}`}>
                  {isEmailValid ? (
                    <>
                      <BadgeCheck className="w-3 h-3" /> Format Verified
                    </>
                  ) : (
                    'Incomplete Format'
                  )}
            ...
            {email.length > 0 && !isEmailValid && (
              <p className="text-[11px] text-amber-600 mt-1 flex items-center gap-1">
                ...
                <span>Please enter a complete institutional email address</span>
              </p>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-slate-400">Min 8 chars, 1 uppercase, 1 numeric, 1 special</span>
            </div>
            <div className="relative">
